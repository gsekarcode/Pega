...
const urgencyBg = (urgency: number) => {
  if (urgency >= 50) return '#fef2f2';
  if (urgency >= 20) return '#fefce8';
  return '#f0fdf4';
};

const urgencyColor = (urgency: number) => {
  if (urgency >= 50) return '#dc2626';
  if (urgency >= 20) return '#ca8a04';
  return '#16a34a';
};

export const StyledUrgencyBadge = styled.span<{ urgency: number }>(({ urgency }) =>
  css`
    display: inline-flex;
    align-items: center;
